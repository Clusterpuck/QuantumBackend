...
# Most functions here can be pruned, but leaving it for now as reference
# Will probably move calculate_cost to the solver, then nuke the utils file
...
def calculate_cost(cost_matrix, solution, circuit):
    cost = 0
    ...
    if circuit:
        cost += cost_matrix[solution[-1], solution[0]]
...
# This is a naive encoding that subdivides the bitstring into n partitions of
# size n representing the selected. If the first partition is [0,1,0],
# then the 2nd node was visited first. This is hugely inefficient,
# but it's a QUBO problem...
...
def points_order_to_binary_state(points_order):
    ...
    # Subdivide BV bitstring into segments
    for segment in range(number_of_points):
        # For segment, set the index corresponding to the solution point's ID
        selected_point = points_order[segment]
        binary_state[number_of_points * segment + selected_point] = 1
